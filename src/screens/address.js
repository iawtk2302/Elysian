...
const Address = () => {
    ...
    useEffect(() => {
       const address= firestore().collection('Addresses').where('userID', '==', fireauth().currentUser.uid).orderBy('selected', 'desc').onSnapshot(onResult, onError);
        return ()=>address()
    }, [])
    return (
        <ScrollView style={{ flex: 1 }}>
            {
                data.map((e) => {
                    return (
                        <ItemAddress item={e} key={e.idAddress} />
                    )
                })
            }
